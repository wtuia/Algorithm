...
public class ArrayUtil {
    ...
    public static void main(String[] args) {
        ...
        arrayUtil.add(arr,9);
        arrayUtil.add(arr,11, true, true);
     ...
     * @param isAppend 是否追加
     */
    private void add(int[] arr, int val, boolean isAppend) {
        add(arr, val, false, false);
     ...
     * @param isSort 是否排序
     */
    private void add(int[] arr, int val, boolean isAppend, boolean isSort) {
        for (int i = 0 , len = delIndex.length; i< len ; i++ ){
            if (delIndex[i] != 0) {
                arr[delIndex[i]] = val;
                delIndex[i] = 0;
                return;
     ...
     * 排序默认关闭
     */
    private void append(int[] arr, int val) {
        arr = append(arr, val ,false);
    }
    private int[] append(int[] arr, int val, boolean isSort) {
        ...
        newArr[len] = val;
        System.out.println("append:" + Arrays.toString(newArr));
        if(isSort){
            sort(arr);
        }
        arr = newArr;
